...
class PlayerCardBox(QWidget):
    def __init__(self, parent=None, spriteID=None):
        super().__init__(parent=parent)
        self.setObjectName("playerCardBox")
    ...
    def configureStyle(self,backgroundColor=None,borderColor=None):
        ...
        self.setStyleSheet(f".PlayerCardBox#playerCardBox {{ border: 3px solid {self.borderColor}; "
    ...
    def setSpriteID(self,spriteID):
        ...
        if(self.__spriteID is None):
            ...
            print(f"{thisPlayer} | {attributeImportanceList}")
            for i,attributeLabel in enumerate(self.attributeLabels):
                attributeLabel.setText(f"{MAPPED_ATTRIBUTES[attributeImportanceList[i]]}: {thisPlayer[attributeImportanceList[i]]}")
