...
class Player:
    ...
    # Theoretically helps with memory?
    __slots__ = ("__spriteID", "_vals", "pmods", "hasPendingUpdates")
    ...
    # Helper organizational map for separating types of values by an arbitrary category.
    valCategories = {
        "Basic Info": ["First_Name",
                    "Last_Name",
                    "NickName",
                    "Archetype",
                    "Rarity",
    ...
    # Default values for every player.
    _DEFAULTS = {"First_Name": "",
                 "Last_Name": "",
                 "NickName": "",
    ...
    def __str__(self):
        return f"{self.__spriteID}| {self['First_Name']} {self['Last_Name']}"
    ...
    def getFullString(self):
        ...
        for key in self._DEFAULTS.keys():
            if key in self.extraValuesMap.values():
                returnString += f"{misc.getKeyFromValue(dictionary=self.extraValuesMap,targetValue=key)}: {self[key]}"
            else:
                returnString += f"{key}: {self[key]}"
            if key in self.idMap.keys():
                returnString += f" ({self.idMap[key][self[key]]})"
            returnString += "\n"
        return returnString
    ...
    def __lt__(self, other):
        return f"{self['First_Name']}{self['Last_Name']}" < f"{other['First_Name']}{other['Last_Name']}"
    ...
    def __getitem__(self,key):
        ...
        if key == "First_Name" or key == "Last_Name":
            return self._vals.get(key,self._DEFAULTS[key])
        ...
        # Testing if the key is an ExtraValue.
            ...
            normalized_key = self.extraValuesMap[key]
            return self._vals.get(normalized_key,self._DEFAULTS[normalized_key])
        # Testing if the key is an ArchetypeName.
        elif key == "Archetype_Name":
            return self._vals.get("Archetype",self._DEFAULTS["Archetype"]).archetypeName
        # Testing if the key is a height represented in some form. (Default form is in inches)
        elif key.startswith("Height"):
            raw_height = self._vals.get("Height",self._DEFAULTS["Height"])
            if key == "HeightFt":
                feet = math.floor(raw_height / 12)
                inches = raw_height - (feet * 12)
                return f"{feet}'{inches}"
            elif key == "HeightCm":
                return round(raw_height * 2.54,2)
        ...
        # Testing for special handedness accessor.
            ...
            hand = self._vals.get("Hand",self._DEFAULTS["Hand"])
            return "Left" if int(hand) == 0 else "Right"
        # Testing if this is a mapped key, ending with _Name
            ...
            if cleanKey in self.idMap.keys():
                return self.idMap[cleanKey][self._vals.get(cleanKey,self._DEFAULTS[cleanKey])]
            else:
    ...
    def __setitem__(self,key,value):
        # Helper for setting/deleting based on delta
        def _set_delta(k,v):
            if k not in self._DEFAULTS:
                er = KeyError(f"Unknown Player key: {k!r}")
                log.exception(er)
                raise er
        ...
        # Testing if the key is an ExtraValue.
            ...
            _set_delta(self.extraValuesMap[key],value)
        # Testing if the key is for Archetype.
            ...
            if type(value) is str or key == "Archetype_Name":
                if value.lower() == "slayer":
                    archetype = archetypes.ARCH_SLAYER
                elif value.lower() == "vigilante":
                    archetype = archetypes.ARCH_VIGILANTE
                elif value.lower() == "medic":
                    archetype = archetypes.ARCH_MEDIC
                elif value.lower() == "guardian":
                    archetype = archetypes.ARCH_GUARDIAN
                elif value.lower() == "engineer":
                    archetype = archetypes.ARCH_ENGINEER
                elif value.lower() == "director":
                    archetype = archetypes.ARCH_DIRECTOR
                elif value.lower() == "none" or value is None:
                    archetype = archetypes.ARCH_NONE
                else:
                ...
                _set_delta("Archetype",archetypes.ARCH_NONE)
            else:
        ...
        # Testing if the key is a height represented in some form. (Default form is in inches)
            ...
            if key == "HeightFt":
                ...
                _set_delta("Height",(feet*12) + inches)
            elif key == "HeightCm":
                _set_delta("Height",int(math.floor(value/2.54)))
            elif key == "HeightIn" or key == "Height":
                _set_delta("Height",int(value))
            else:
        ...
        # Testing if the key is a mapped value
            ...
            cleanKey = key.split("_Name")[0]
            if cleanKey not in self.idMap:
                error = ValueError(f"'{cleanKey}' is not a mapped value.")
                log.exception(error)
                raise error
            if value in self.idMap[cleanKey]:
                _set_delta(cleanKey,self.idMap[cleanKey].index(value))
            elif type(value) is int:
                _set_delta(cleanKey,value)
            elif key.endswith("_Name"):
        ...
        # Test if it's an attribute:
            ...
            bounded_int = int(value)
            bounded_int = 99 if bounded_int > 99 else bounded_int
            bounded_int = 25 if bounded_int < 25 else bounded_int
            if key == "SShtIns" and bounded_int < 60:
                bounded_int = 60
            elif key == "SShtCls" and bounded_int < 50:
                bounded_int = 50
            _set_delta(key,bounded_int)
    ...
    def genArchetype(self):
        self["Archetype"] = random.choice(["Slayer","Vigilante","Medic","Guardian","Engineer","Director"])
    ...
    def genAttributes(self,archetype = None):
        ...
        self["SOffHDrib"] = random.randrange(archetype.attributeRanges.get("SOffHDrib")[0], archetype.attributeRanges.get("SOffHDrib")[1] + 1)
        self["SHands"] = random.randrange(archetype.attributeRanges.get("SHands")[0], archetype.attributeRanges.get("SHands")[1] + 1)
        self["SOAwar"] = random.randrange(archetype.attributeRanges.get("SOAwar")[0], archetype.attributeRanges.get("SOAwar")[1] + 1)
        self["SBallHndl"] = random.randrange(archetype.attributeRanges.get("SBallHndl")[0], archetype.attributeRanges.get("SBallHndl")[1] + 1)
        self["SBallSec"] = random.randrange(archetype.attributeRanges.get("SBallSec")[0], archetype.attributeRanges.get("SBallSec")[1] + 1)
        self["SPass"] = random.randrange(archetype.attributeRanges.get("SPass")[0], archetype.attributeRanges.get("SPass")[1] + 1)
        self["SSpeed"] = random.randrange(archetype.attributeRanges.get("SSpeed")[0], archetype.attributeRanges.get("SSpeed")[1] + 1)
        self["SQuick"] = random.randrange(archetype.attributeRanges.get("SQuick")[0], archetype.attributeRanges.get("SQuick")[1] + 1)
        self["SHustle"] = random.randrange(archetype.attributeRanges.get("SHustle")[0], archetype.attributeRanges.get("SHustle")[1] + 1)
        self["SSteal"] = random.randrange(archetype.attributeRanges.get("SSteal")[0], archetype.attributeRanges.get("SSteal")[1] + 1)
        self["SDLowPost"] = random.randrange(archetype.attributeRanges.get("SDLowPost")[0], archetype.attributeRanges.get("SDLowPost")[1] + 1)
        self["SStrength"] = random.randrange(archetype.attributeRanges.get("SStrength")[0], archetype.attributeRanges.get("SStrength")[1] + 1)
        self["SBlock"] = random.randrange(archetype.attributeRanges.get("SBlock")[0], archetype.attributeRanges.get("SBlock")[1] + 1)
        self["SOnBallD"] = random.randrange(archetype.attributeRanges.get("SOnBallD")[0], archetype.attributeRanges.get("SOnBallD")[1] + 1)
        self["SOReb"] = random.randrange(archetype.attributeRanges.get("SOReb")[0], archetype.attributeRanges.get("SOReb")[1] + 1)
        self["SDReb"] = random.randrange(archetype.attributeRanges.get("SDReb")[0], archetype.attributeRanges.get("SDReb")[1] + 1)
        self["SDAwar"] = random.randrange(archetype.attributeRanges.get("SDAwar")[0], archetype.attributeRanges.get("SDAwar")[1] + 1)
        self["SShtIns"] = random.randrange(archetype.attributeRanges.get("SShtIns")[0], archetype.attributeRanges.get("SShtIns")[1] + 1)
        self["SDunk"] = random.randrange(archetype.attributeRanges.get("SDunk")[0], archetype.attributeRanges.get("SDunk")[1] + 1)
        self["SStdDunk"] = random.randrange(archetype.attributeRanges.get("SStdDunk")[0], archetype.attributeRanges.get("SStdDunk")[1] + 1)
        self["SVertical"] = random.randrange(archetype.attributeRanges.get("SVertical")[0], archetype.attributeRanges.get("SVertical")[1] + 1)
        self["SShtFT"] = random.randrange(archetype.attributeRanges.get("SShtFT")[0], archetype.attributeRanges.get("SShtFT")[1] + 1)
        self["SStamina"] = random.randrange(archetype.attributeRanges.get("SStamina")[0], archetype.attributeRanges.get("SStamina")[1] + 1)
        self["SDurab"] = random.randrange(archetype.attributeRanges.get("SDurab")[0], archetype.attributeRanges.get("SDurab")[1] + 1)
        self["SPOT"] = random.randrange(archetype.attributeRanges.get("SPOT")[0], archetype.attributeRanges.get("SPOT")[1] + 1)
        self["SShtCls"] = random.randrange(archetype.attributeRanges.get("SShtCls")[0], archetype.attributeRanges.get("SShtCls")[1] + 1)
        self["SLayUp"] = random.randrange(archetype.attributeRanges.get("SLayUp")[0], archetype.attributeRanges.get("SLayUp")[1] + 1)
        self["SPstFdaway"] = random.randrange(archetype.attributeRanges.get("SPstFdaway")[0], archetype.attributeRanges.get("SPstFdaway")[1] + 1)
        self["SPstHook"] = random.randrange(archetype.attributeRanges.get("SPstHook")[0], archetype.attributeRanges.get("SPstHook")[1] + 1)
        self["SOLowPost"] = random.randrange(archetype.attributeRanges.get("SOLowPost")[0], archetype.attributeRanges.get("SOLowPost")[1] + 1)
        self["SShtMed"] = random.randrange(archetype.attributeRanges.get("SShtMed")[0], archetype.attributeRanges.get("SShtMed")[1] + 1)
        self["SSht3PT"] = random.randrange(archetype.attributeRanges.get("SSht3PT")[0], archetype.attributeRanges.get("SSht3PT")[1] + 1)
        self["SShtInT"] = random.randrange(archetype.attributeRanges.get("SShtInT")[0], archetype.attributeRanges.get("SShtInT")[1] + 1)
        self["SShtOfD"] = random.randrange(archetype.attributeRanges.get("SShtOfD")[0], archetype.attributeRanges.get("SShtOfD")[1] + 1)
        self["SConsis"] = random.randrange(archetype.attributeRanges.get("SConsis")[0], archetype.attributeRanges.get("SConsis")[1] + 1)
    ...
    def genTendencies(self,archetype = None):
        ...
        self["TShtTend"] = random.randrange(archetype.t_ShotTnd[0], archetype.t_ShotTnd[1] + 1)
        self["TInsShots"] = random.randrange(archetype.t_InsideShot[0], archetype.t_InsideShot[1] + 1)
        self["TCloseSht"] = random.randrange(archetype.t_CloseShot[0], archetype.t_CloseShot[1] + 1)
        self["TMidShots"] = random.randrange(archetype.t_MidShot[0], archetype.t_MidShot[1] + 1)
        self["T3PTShots"] = random.randrange(archetype.t_ShotThreePt[0], archetype.t_ShotThreePt[1] + 1)
        self["TDriveLn"] = random.randrange(archetype.t_DriveLane[0], archetype.t_DriveLane[1] + 1)
        self["TDriveRvL"] = random.randrange(archetype.t_DriveRight[0], archetype.t_DriveRight[1] + 1)
        self["TPullUp"] = random.randrange(archetype.t_PullUp[0], archetype.t_PullUp[1] + 1)
        self["TPumpFake"] = random.randrange(archetype.t_PumpFake[0], archetype.t_PumpFake[1] + 1)
        self["TTrplThrt"] = random.randrange(archetype.t_TripleThreat[0], archetype.t_TripleThreat[1] + 1)
        self["TNoTT"] = random.randrange(archetype.t_NoTripleThreat[0], archetype.t_NoTripleThreat[1] + 1)
        self["TTTShot"] = random.randrange(archetype.t_TripleThreatShot[0], archetype.t_TripleThreatShot[1] + 1)
        self["TSizeUp"] = random.randrange(archetype.t_Sizeup[0], archetype.t_Sizeup[1] + 1)
        self["THesitat"] = random.randrange(archetype.t_Hesitation[0], archetype.t_Hesitation[1] + 1)
        self["TStrghtDr"] = random.randrange(archetype.t_StraightDribble[0], archetype.t_StraightDribble[1] + 1)
        self["TCrossov"] = random.randrange(archetype.t_Crossover[0], archetype.t_Crossover[1] + 1)
        self["TSpin"] = random.randrange(archetype.t_Spin[0], archetype.t_Spin[1] + 1)
        self["TStepBack"] = random.randrange(archetype.t_Stepback[0], archetype.t_Stepback[1] + 1)
        self["THalfSpin"] = random.randrange(archetype.t_Halfspin[0], archetype.t_Halfspin[1] + 1)
        self["TDblCross"] = random.randrange(archetype.t_DoubleCrossover[0], archetype.t_DoubleCrossover[1] + 1)
        self["TBhndBack"] = random.randrange(archetype.t_BehindBack[0], archetype.t_BehindBack[1] + 1)
        self["THesCross"] = random.randrange(archetype.t_HesitationCross[0], archetype.t_HesitationCross[1] + 1)
        self["TInAndOut"] = random.randrange(archetype.t_InNOut[0], archetype.t_InNOut[1] + 1)
        self["TDPSimpDr"] = random.randrange(archetype.t_SimpleDrive[0], archetype.t_SimpleDrive[1] + 1)
        self["TAttackB"] = random.randrange(archetype.t_Attack[0], archetype.t_Attack[1] + 1)
        self["TPassOut"] = random.randrange(archetype.t_PassOut[0], archetype.t_PassOut[1] + 1)
        self["THopStep"] = random.randrange(archetype.t_Hopstep[0], archetype.t_Hopstep[1] + 1)
        self["TSpinLUp"] = random.randrange(archetype.t_SpinLayup[0], archetype.t_SpinLayup[1] + 1)
        self["TEuroStep"] = random.randrange(archetype.t_Eurostep[0], archetype.t_Eurostep[1] + 1)
        self["TRunner"] = random.randrange(archetype.t_Runner[0], archetype.t_Runner[1] + 1)
        self["TFadeaway"] = random.randrange(archetype.t_Fadeaway[0], archetype.t_Fadeaway[1] + 1)
        self["TDunkvLU"] = random.randrange(archetype.t_Dunk[0], archetype.t_Dunk[1] + 1)
        self["TVShCrash"] = random.randrange(archetype.t_Crash[0], archetype.t_Crash[1] + 1)
        self["TTouches"] = random.randrange(archetype.t_Touches[0], archetype.t_Touches[1] + 1)
        self["TUsePick"] = random.randrange(archetype.t_UsePick[0], archetype.t_UsePick[1] + 1)
        self["TSetPick"] = random.randrange(archetype.t_SetPick[0], archetype.t_SetPick[1] + 1)
        self["TIsolat"] = random.randrange(archetype.t_Isolation[0], archetype.t_Isolation[1] + 1)
        self["TUseOBScr"] = random.randrange(archetype.t_UseOffBallScreen[0], archetype.t_UseOffBallScreen[1] + 1)
        self["TSetOBScr"] = random.randrange(archetype.t_SetOffBallScreen[0], archetype.t_SetOffBallScreen[1] + 1)
        self["TPostUp"] = random.randrange(archetype.t_PostUp[0], archetype.t_PostUp[1] + 1)
        self["TSpotUp"] = random.randrange(archetype.t_SpotUp[0], archetype.t_SpotUp[1] + 1)
        self["TPostSpn"] = random.randrange(archetype.t_PostSpin[0], archetype.t_PostSpin[1] + 1)
        self["TPostDrpSt"] = random.randrange(archetype.t_DropStep[0], archetype.t_DropStep[1] + 1)
        self["TPostShmSh"] = random.randrange(archetype.t_Shimmy[0], archetype.t_Shimmy[1] + 1)
        self["TPostFaceU"] = random.randrange(archetype.t_FaceUp[0], archetype.t_FaceUp[1] + 1)
        self["TLeavePost"] = random.randrange(archetype.t_LeavePost[0], archetype.t_LeavePost[1] + 1)
        self["TPostBDown"] = random.randrange(archetype.t_BackDown[0], archetype.t_BackDown[1] + 1)
        self["TPostAgBd"] = random.randrange(archetype.t_AggressiveBackDown[0], archetype.t_AggressiveBackDown[1] + 1)
        self["TPostShots"] = random.randrange(archetype.t_PostShot[0], archetype.t_PostShot[1] + 1)
        self["TPostHook"] = random.randrange(archetype.t_PostHook[0], archetype.t_PostHook[1] + 1)
        self["TPostFdawy"] = random.randrange(archetype.t_PostFade[0], archetype.t_PostFade[1] + 1)
        self["TPostDrv"] = random.randrange(archetype.t_PostDrive[0], archetype.t_PostDrive[1] + 1)
        self["TPostHopSh"] = random.randrange(archetype.t_HopShot[0], archetype.t_HopShot[1] + 1)
        self["TPutbacks"] = random.randrange(archetype.t_Putback[0], archetype.t_Putback[1] + 1)
        self["TFlshPass"] = random.randrange(archetype.t_FlashyPass[0], archetype.t_FlashyPass[1] + 1)
        self["TThrowAO"] = random.randrange(archetype.t_AlleyOop[0], archetype.t_AlleyOop[1] + 1)
        self["TDrawFoul"] = random.randrange(archetype.t_DrawFoul[0], archetype.t_DrawFoul[1] + 1)
        self["TPassLane"] = random.randrange(archetype.t_PlayPassLane[0], archetype.t_PlayPassLane[1] + 1)
        self["TTakeChrg"] = random.randrange(archetype.t_TakeCharge[0], archetype.t_TakeCharge[1] + 1)
        self["TOnBalStl"] = random.randrange(archetype.t_OnBallSteal[0], archetype.t_OnBallSteal[1] + 1)
        self["TContShot"] = random.randrange(archetype.t_Contest[0], archetype.t_Contest[1] + 1)
        self["TCommFoul"] = random.randrange(archetype.t_CommitFoul[0], archetype.t_CommitFoul[1] + 1)
        self["THardFoul"] = random.randrange(archetype.t_HardFoul[0], archetype.t_HardFoul[1] + 1)
        self["TUseGlass"] = random.randrange(archetype.t_UseGlass[0], archetype.t_UseGlass[1] + 1)
        self["TStpbJmpr"] = random.randrange(archetype.t_StepbackJumper[0], archetype.t_StepbackJumper[1] + 1)
        self["TSpinJmpr"] = random.randrange(archetype.t_SpinJumper[0], archetype.t_SpinJumper[1] + 1)
        self["TStpThrgh"] = random.randrange(archetype.t_StepThrough[0], archetype.t_StepThrough[1] + 1)
        self["TAlleyOop"] = random.randrange(archetype.t_ThrowAlleyOop[0], archetype.t_ThrowAlleyOop[1] + 1)
        self["TGiveGo"] = random.randrange(archetype.t_GiveNGo[0], archetype.t_GiveNGo[1] + 1)
    ...
    def genHotspots(self,archetype = None):
        ...
        # Isolation
        isoChoices = weighted_dict.WeightedDict()
        ...
        # Pick and Roll
        pickRollChoices = weighted_dict.WeightedDict()
        ...
        # Spot Up
        spotUpChoices = weighted_dict.WeightedDict()
        ...
        # Post
        postChoices = weighted_dict.WeightedDict()
    ...
    def genHeight(self,archetype = None):
        if archetype is None:
            if "Archetype" not in self._vals:
                ...
                archetype = self["Archetype"]
        self["Height"] = random.randrange(archetype.heightRange[0],archetype.heightRange[1] + 1)
    ...
    def genAnimations(self,archetype = None,dunkCount : int = None):
        ...
        self["AShtForm"] = random.randrange(0, len(self.idMap["AShtForm"]))
        self["AShtBase"] = random.randrange(0, len(self.idMap["AShtBase"]))
        self["AFadeaway"] = random.randrange(0, len(self.idMap["AFadeaway"]))
        self["AContestd"] = random.randrange(0,len(self.idMap["AContestd"]))
        self["AEscDrPlU"] = random.randrange(0,len(self.idMap["AEscDrPlU"]))
        self["ARunner"] = random.randrange(0, len(self.idMap["ARunner"]))
        self["AFreeT"] = random.randrange(0, len(self.idMap["AFreeT"]))
        self["ADrPullUp"] = random.randrange(0, len(self.idMap["ADrPullUp"]))
        self["ASpinJmpr"] = random.randrange(0, len(self.idMap["ASpinJmpr"]))
        self["AHopJmpr"] = random.randrange(0, len(self.idMap["AHopJmpr"]))
        ...
        self["APstFade"] = random.choice(postFadeOptions)
        ...
        self["APstHook"] = random.choice(postHookOptions)

        self["APstHopSh"] = random.randrange(0, len(self.idMap["APstHopSh"]))
        self["APstShmSh"] = random.randrange(0, len(self.idMap["APstShmSh"]))
        self["APstDrvStB"] = random.randrange(0, len(self.idMap["APstDrvStB"]))
        self["APstSpnStB"] = random.randrange(0, len(self.idMap["APstSpnStB"]))
        self["APstPrtct"] = random.randrange(0, len(self.idMap["APstPrtct"]))
        self["APstPrtSpn"] = random.randrange(0, len(self.idMap["APstPrtSpn"]))
        self["AIsoCross"] = random.randrange(0, len(self.idMap["AIsoCross"]))
        self["AIsoBhBck"] = random.randrange(0, len(self.idMap["AIsoBhBck"]))
        self["AIsoSpin"] = random.randrange(0, len(self.idMap["AIsoSpin"]))
        self["AIsoHesit"] = random.randrange(0, len(self.idMap["AIsoHesit"]))
        ...
        self["ALayUp"] = random.choice(layUpOptions)

        if dunkCount is None:
            if self["SDunk"] <= 49:
                dunkCount = 1
            elif 49 < self["SDunk"] <= 70:
                dunkCount = 2
            elif 70 < self["SDunk"] <= 80:
                dunkCount = 3
            elif 80 < self["SDunk"] <= 90:
                dunkCount = 4
            elif 90 < self["SDunk"] <= 99:
                dunkCount = 5
        ...
        if dunkCount >= 1:
            ...
            self["AGoToDunk"] = thisDunk
        if dunkCount >= 2:
            thisDunk = random.choice(dunkOptions)
            dunkOptions.remove(thisDunk)
            self["ADunk2"] = thisDunk
        if dunkCount >= 3:
            thisDunk = random.choice(dunkOptions)
            dunkOptions.remove(thisDunk)
            self["ADunk3"] = thisDunk
        if dunkCount >= 4:
            thisDunk = random.choice(dunkOptions)
            dunkOptions.remove(thisDunk)
            self["ADunk4"] = thisDunk
        if dunkCount >= 5:
            thisDunk = random.choice(dunkOptions)
            dunkOptions.remove(thisDunk)
            self["ADunk5"] = thisDunk

        self["AIntHLght"] = random.randrange(0, len(self.idMap["AIntHLght"]))
        self["AIntPreG1"] = random.randrange(0, len(self.idMap["AIntPreG1"]))
        self["AIntPreG2"] = random.randrange(0, len(self.idMap["AIntPreG2"]))
        self["AIntPreT1"] = random.randrange(0, len(self.idMap["AIntPreT1"]))
        self["AIntPreT2"] = random.randrange(0, len(self.idMap["AIntPreT2"]))
    ...
    def genPlayStyle(self,archetype = None):
        ...
        if archetype.archetypeName == "Slayer":
            ...
            if self["SDunk"] >= 80 or self["SLayUp"] >= 75:
            ...
            if self["SShtMed"] >= 80 or self["SShtCls"] >= 90:
        ...
        self["PlayStyle"] = random.choice(potentialStyles)
    ...
    def genPlayTypes(self, archetype = None):
        ...
        if archetype.archetypeName == "Director" or archetype.archetypeName == "Engineer":
            self["PlayType1"] = random.choice(playTypeChoices)
            playTypeChoices.remove(self["PlayType1"])
            self["PlayType2"] = random.choice(playTypeChoices)
        else:
            self["PlayType1"] = 0
            self["PlayType2"] = 0
        self["PlayType3"] = 0
    ...
    def genMisc(self):
        self["Personality"] = random.randrange(0,len(self.idMap["Personality"]))
        self.hasPendingUpdates = True
